...
from scenic.core.regions import PolygonalRegion, PolylineRegion, regionFromShapelyObject
...
class NuscQueryAPI:
	...
	VEHICLE_CATEGORIES = {'vehicle.car', 'vehicle.truck', 'vehicle.emergency.police'}
	INVALID_CATEGORIES = {'movable_object.trafficcone','movable_object.pushable_pullable','movable_object.debris', \
	'movable_object.barrier','human.pedestrian.wheelchair','human.pedestrian.stroller','human.pedestrian.police_officer', \
	'human.pedestrian.personal_mobility', 'human.pedestrian.construction_worker', 'human.pedestrian.child', 'human.pedestrian.adult', 'animal', \
	'vehicle.bicycle', 'vehicle.bus.rigid', 'vehicle.bus.bendy','vehicle.motorcycle','vehicle.trailer','vehicle.emergency.ambulance', \
	...
	def get_img_data(self, img_filename):
		...
		road = regionFromShapelyObject(self.get_whole_map(location)['road'])
		...
		for ann_rec in ann_recs:
			...
			# Filter out all labels regarding entities NOT on road
			if not road.containsPoint(Vector(xy[0], xy[1])):
				continue
			...
			if category['name'] not in self.VEHICLE_CATEGORIES:
				continue
			...
			vehicle_dict = {'heading': heading, 'position': tuple(xy), 'box': box_2d}
			vehicles.append(vehicle_dict)

		# Sort vehicles by distance to ego
		vehicles.sort(key=lambda v: (v['position'][0] - ego_xy[0]) ** 2 + (v['position'][1] - ego_xy[1]) ** 2)

		ego_dict = {'heading': ego_heading, 'position': ego_xy}
		retval = {'EgoCar': ego_dict, 'Vehicles': vehicles}
